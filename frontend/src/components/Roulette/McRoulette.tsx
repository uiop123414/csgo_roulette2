...
const McRoulette = ({ transitionDuration }: RouletteElementParams) => {
  ...
  async function getWinner() {
    ...
    const response = await axios.post(
      "http://127.0.0.1:8000/api/run_roulette?" + new URLSearchParams({ slot_name: "CSGO_1" }),
      {},
      { headers: headers },
    );
  ...
  const renderButton = () => {
    if (localStorage.getItem("authTokens") === null)
      return (
        <Grid container spacing={3} id="grid">
          <Grid item xs={4}>
            <button className={cl.button} disabled={true}>
              Roll
            </button>
          </Grid>
        </Grid>
    ...
    return (
      <Grid container spacing={3} id="grid">
        <Grid item xs={4}>
          <button className={cl.button} disabled={isSpin} onClick={async () => await getWinner()}>
            Roll
          </button>
        </Grid>
      </Grid>
